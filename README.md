...
- **界面语言：英文。** 目标市场是北美，站内所有面向用户的文案、题目、AI 反馈一律英文；代码注释也用英文保持一致。本 README 作为内部设计文档保持中文。
  - 分类取值：`ML Fundamentals` / `Deep Learning` / `LLM / GenAI` / `ML System Design` / `Coding` / `Behavioral`
  - 难度取值：`Easy` / `Medium` / `Hard`
  - 这两组值是**落库的数据**，不是显示层翻译。以后要做多语言得另加 i18n 层，不能直接改这些字符串。
